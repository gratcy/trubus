...
class Opnamecustomer_model extends CI_Model {
	...
	function __get_stock_adjustment($iid, $branch, $type) {
		if ($type == 1)
			$this -> db -> select('SUM(oadjustplus) as total FROM opname_tab WHERE otype=2 AND oidid='.$iid.' AND obid=' . $branch);
		else
			$this -> db -> select('SUM(oadjustmin) as total FROM opname_tab WHERE otype=2 AND oidid='.$iid.' AND obid=' . $branch);
	...
	function __get_opname_inventory($cid) {
		return 'SELECT a.iid,a.ibid,a.istockbegining,a.istockin,a.istockout,a.istockreject,a.istockretur,a.istock,a.istatus,b.btitle,c.cid,c.cname FROM inventory_tab a LEFT JOIN books_tab b ON a.ibid=b.bid LEFT JOIN customer_tab c ON a.ibcid=c.cid WHERE a.itype=2 AND (a.istatus=1 OR a.istatus=0) AND b.bstatus=1 AND a.ibcid='.$cid.' ORDER BY a.iid DESC';
	}
