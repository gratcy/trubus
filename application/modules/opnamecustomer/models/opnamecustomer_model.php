...
class Opnamecustomer_model extends CI_Model {
	...
	function __get_customer($ctype=1) {
		if ($ctype) $ctype = ' AND a.ctype=1';
		return 'SELECT a.*,b.bname,c.bname as bgname,d.aname FROM customer_tab a LEFT JOIN branch_tab b ON a.cbid=b.bid LEFT JOIN books_group_tab c ON a.cgroup=c.bid LEFT JOIN area_tab d ON a.carea=d.aid WHERE a.cstatus=1'.$ctype.' ORDER BY a.cid DESC';
	}
