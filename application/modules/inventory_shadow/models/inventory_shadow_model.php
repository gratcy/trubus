...
class inventory_shadow_model extends CI_Model {
	...
	function __get_inventory_shadow($bid="") {
		...
		return 'SELECT a.iid,a.ibid,a.ibcid,a.istockbegining,a.istockin,a.istockout,a.istockretur,a.istockreject,a.istock,a.istatus,b.btitle FROM inventory_shadow_tab a LEFT JOIN books_tab b ON a.ibid=b.bid WHERE (a.istatus=1 OR a.istatus=0)'.$bid.' ORDER BY a.iid DESC';
	}
	
	function __get_search($book, $bid="") {
		if ($bid != "") $bid = " AND a.ibcid=" . $bid;
		else $bid = "";
		return 'SELECT a.iid,a.ibid,a.ibcid,a.istockbegining,a.istockin,a.istockout,a.istockretur,a.istockreject,a.istock,a.istatus,b.btitle FROM inventory_shadow_tab a LEFT JOIN books_tab b ON a.ibid=b.bid WHERE (a.istatus=1 OR a.istatus=0) AND a.ibid='.$book.''.$bid.' ORDER BY a.iid DESC';
	}
