...
class retur_bk_detail_model extends CI_Model {
	...
	function __get_retur_bk_detailxx($id) {
		$sql=$this -> db -> query( 'SELECT *,(select cname from customer_tab b where b.cid=a.tcid)as cname FROM transaction_tab a WHERE (a.tstatus=1 OR a.tstatus=0) AND ttype=3 AND ttypetrans=4   AND a.tid=' . $id .'');
		return $sql-> result();
	}
	...
	function __insert_retur_bk_detailp($data) {
	//print_r($data);die;
        return $this -> db -> insert('trans_tab', $data);
	}

	
function __update_retur_bks($tid,$data) {
	//$year=date('Y');
	//print_r($data);
 ...
 //return $this -> db -> update('transaction_tab', $data);
	...
	function __update_retur_bk_detail($id, $data) {
