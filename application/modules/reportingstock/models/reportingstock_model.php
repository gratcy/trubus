...
class Reportingstock_model extends CI_Model {
    ...
    function __get_transaction_summary(&$arr) {
		...
		if($arr['typei'] == 'RB') $tpi = "OR a.tnofaktur LIKE 'RB%' ";
		else $tpi = "";
		...
		if($arr['typei'] == 'RB') {
			$this -> db -> select($fild."c.acode,a.tnofaktur, a.tnospo,a.ttanggal,c.aname, SUM(d.ttharga) as bruto, SUM(d.ttotal) as netto, SUM(d.tqty) as totalqty FROM transaction_tab a LEFT JOIN customer_tab b ON a.tcid=b.cid LEFT JOIN area_tab c ON b.carea=c.aid LEFT JOIN publisher_tab p ON a.tpid=p.pid LEFT JOIN transaction_detail_tab d ON a.tid=d.ttid LEFT JOIN books_tab e ON d.tbid=e.bid WHERE a.tstatus!=2 AND (a.ttanggal >= '".$dsa."' AND a.ttanggal <= '".$dsb."') AND a.tbid=".$arr['branchid']."".$appr.$tpx.$pub.$kb.$groupby, FALSE);
		}
		else {
			$this -> db -> select($fild."a.tnofaktur, a.tnospo,a.ttanggal,b.ccode,b.cname,c.acode, c.aname, SUM(d.ttharga) as bruto, SUM(d.ttotal) as netto, SUM(d.tqty) as totalqty FROM transaction_tab a LEFT JOIN customer_tab b ON a.tcid=b.cid LEFT JOIN area_tab c ON b.carea=c.aid LEFT JOIN transaction_detail_tab d ON a.tid=d.ttid LEFT JOIN books_tab e ON d.tbid=e.bid WHERE a.tstatus!=2 AND (a.ttanggal >= '".date('Y-m-d',strtotime($dsa))."' AND a.ttanggal <= '".date('Y-m-d',strtotime($dsb))."') AND c.abid=".$arr['branchid']." AND a.tbid=".$arr['branchid']."".$appr.$cus.$area.$tpx.$pub.$kb.$groupby, FALSE);
		}
		return $this -> db -> get() -> result();
	}
	...
	function __get_transfer_record($bid,$dfrom,$dto,$kode_buku,$kode_bukux,$rtype,$publisher,$approval) {
		...
		$this -> db -> select("a.ddrid,a.dtype,a.ddocno as tnofaktur,a.ddesc as ket,from_unixtime(a.ddate,'%Y-%m-%d') as ttanggal,c.dqty as tqty,d.btitle,d.bcode,d.bprice,'0' as tdisc,'0' as ttharga,e.pname,f.bname as narea,f.bname as cname,f.bcode as ccode,'0' as ttotal".$rtype." FROM distribution_tab a LEFT JOIN distribution_request_tab b ON a.ddrid=b.did LEFT JOIN branch_tab f ON b.dbfrom=f.bid LEFT JOIN distribution_book_tab c ON a.ddrid=c.ddrid LEFT JOIN books_tab d ON c.dbid=d.bid LEFT JOIN publisher_tab e ON d.bpublisher=e.pid WHERE c.dstatus=1 AND (b.dbto=".$bid." OR b.dbfrom=".$bid.")".$approval." AND c.dstatus=1 AND (from_unixtime(a.ddate,'%Y-%m-%d') >= '".date('Y-m-d',strtotime($dfrom))."' AND from_unixtime(a.ddate,'%Y-%m-%d') <= '".date('Y-m-d',strtotime($dto))."')".$kb.$pub, FALSE);
		return $this -> db -> get() -> result();
	}
	...
	function __get_request_record($bid,$dfrom,$dto,$kode_buku,$kode_bukux,$rtype,$publisher,$approval) {
		...
		$this -> db -> select("a.did,a.ddesc as ket,a.dtype,from_unixtime(a.ddate,'%Y-%m-%d') as ttanggal,a.dtitle,a.ddesc,a.dstatus,b.bcode as ccode,b.bname as narea,c.bname as cname,d.dqty as tqty,e.btitle,e.bcode,e.bprice,'0' as tdisc,'0' as ttharga,f.pname,'0' as ttotal".$rtype." FROM distribution_request_tab a LEFT JOIN branch_tab b ON a.dbfrom=b.bid LEFT JOIN branch_tab c ON a.dbto=c.bid INNER JOIN distribution_book_tab d ON a.did=d.ddrid INNER JOIN books_tab e ON d.dbid=e.bid LEFT JOIN publisher_tab f ON e.bpublisher=f.pid WHERE (a.dbfrom=".$bid." OR a.dbto=".$bid.")".$approval." AND (from_unixtime(a.ddate,'%Y-%m-%d') >= '".date('Y-m-d',strtotime($dfrom))."' AND from_unixtime(a.ddate,'%Y-%m-%d') <= '".date('Y-m-d',strtotime($dto))."') AND d.dstatus=1".$kb.$pub." ORDER BY a.did DESC", FALSE);
	...
	function __get_receiving_record($bid,$dfrom,$dto,$kode_buku,$kode_bukux,$rtype,$publisher,$approval) {
		...
		else $kb = " AND b.rbid IN (".implode(",",$kode_buku).") ";
		...
		$this -> db -> select("a.rdocno as tnofaktur,a.rdesc as ket,from_unixtime(a.rdate,'%Y-%m-%d') as ttanggal,b.rqty as tqty,c.btitle,c.bcode,c.bprice,d.pid,d.pname,'0' as tdisc,'0','0' as ttotal,'0' as ttharga".$rtype." FROM receiving_tab a LEFT JOIN receiving_books_tab b ON a.rid=b.rrid LEFT JOIN books_tab c ON b.rbid=c.bid LEFT JOIN publisher_tab d ON c.bpublisher=d.pid WHERE a.rtype=2".$approval." AND a.rbid=".$bid." AND (from_unixtime(a.rdate,'%Y-%m-%d') >= '".date('Y-m-d',strtotime($dfrom))."' AND from_unixtime(a.rdate,'%Y-%m-%d') <= '".date('Y-m-d',strtotime($dto))."') AND b.rstatus=1".$kb.$pub, FALSE);
		return $this -> db -> get() -> result();
	}
}
