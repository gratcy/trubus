...
class Books_model extends CI_Model {
	...
	function __get_last_book_by_publisher($cid) {
		$this -> db -> select('bcode FROM books_tab WHERE (bstatus=1 OR bstatus=0) AND bpublisher=' . $cid . ' ORDER BY bcode DESC LIMIT 1');
		return $this -> db -> get() -> result();
	}
	...
	function __get_books_search_inventory($keyword) {
		...
		$this -> db -> select("a.bid FROM books_tab a LEFT JOIN publisher_tab b ON a.bpublisher=b.pid WHERE a.bstatus=1 AND (a.btitle LIKE '%".$keyword."%' OR a.bcode LIKE '%".$keyword."%' OR b.pname LIKE '%".$keyword."%' OR a.bprice LIKE '%".$keyword."%')");
		return $this -> db -> get() -> result();
	}
