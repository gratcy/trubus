...
class retur_jk_model extends CI_Model {
	...
	function __get_retur_jk() {
		...
		return "SELECT a.*,b.cname FROM transaction_tab a LEFT JOIN customer_tab b ON a.tcid=b.cid WHERE (a.tstatus='1' OR a.tstatus='0') AND a.ttype='1' AND a.ttypetrans='4' and a.tbid='$branch' ORDER BY a.tid DESC";
	}
	
	function __get_retur_jk_search($keyword) {
		$branch=$this -> memcachedlib -> sesresult['ubranchid'];
		return "SELECT a.*,b.cname FROM transaction_tab a LEFT JOIN customer_tab b ON a.tcid=b.cid WHERE (a.tnofaktur LIKE '%".$keyword."%' OR b.cname LIKE '%".$keyword."%' OR a.tinfo LIKE '%".$keyword."%') AND (a.tstatus='1' OR a.tstatus='0') AND a.ttype='1' AND a.ttypetrans='4' and a.tbid='$branch'ORDER BY a.tid DESC";
	}
    ...
    function __get_hasil_penjualan_by_date($datefrom,$dateto) {
		...
		FROM transaction_tab a,transaction_detail_tab b, customer_tab e WHERE a.tcid=e.cid AND (a.ttanggal between '$datefrom' and '$dateto') and a.tid=b.ttid AND a.tbid='$branch' AND a.ttype='1'  AND a.ttypetrans='4' AND a.tstatus='1' ");
		return $sql -> result();
	}
